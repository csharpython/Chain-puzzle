...
export default (scenes) => {
     ...
     */
    const objectSome = (obj, func) => Object.keys(obj).some(x => func(obj[x]));
    const endscene = (err = null, wasSuccess = false) => {
    ...
    const gameClear = () => {
        if (DATA.target.score < 0 ||
            objectSome(DATA.target?.obj, (x) => x > 0) ||
            objectSome(DATA.target?.field, (x) => x > 0))
            return false;
        alert("GAME CLEAR");
        endscene(null, true);
        return true;
    };
    const addScore = (score_mult, base = BASE_SCORE) => (DATA.target.score += Math.floor(base * score_mult));
    const getType = (obj) => obj[0];
    const getCellPos = (cell) => [cell.target.parentNode.rowIndex, cell.target.cellIndex];
    ...
    const update_cell = (y, x) => {
        ...
        const IMGOBJ = CELL.querySelector("img.object");
        const IMG_FIELD = CELL.querySelector("img.field");
    ...
    const chain_connect = (cell) => {
        if (game_state !== ENUM_STATUS.CHAINING)
            return;
        const [CELL_Y, CELL_X] = getCellPos(cell);
        const CELL_COLOR = getType(DATA.board.obj[CELL_Y][CELL_X]);
        const CHAIN_LAST = chain_yx.at(-1);
        if (Math.abs(CHAIN_LAST.y - CELL_Y) > 1)
            return;
        if (Math.abs(CHAIN_LAST.x - CELL_X) > 1)
            return; /*位置チェック*/
        if (chain_color !== CELL_COLOR)
            return;
        if (chain_yx.some(e => e.x === CELL_X && e.y === CELL_Y))
            return; /*条件チェック*/
    ...
    const load_board = () => {
        ...
        if (HEIGHT <= 0)
            endscene(RangeError(`GUARD! Height : ${HEIGHT} isn't positive`));
        if (WIDTH <= 0)
            endscene(RangeError(`GUARD! Width : ${WIDTH} isn't positive`));
    ...
    const startgame = (StageID) => {
        ...
        if (isNaN(INT_ARG))
            endscene(TypeError(`GUARD! StageID ${StageID} is NaN`));
