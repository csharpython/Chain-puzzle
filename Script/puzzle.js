export default scenes => {
	...
	const update_cell = (y, x) => {
		...
		[
			CELL.querySelector("img.object").src,
			CELL.querySelector("img.field").src,
			CELL.querySelector("img.object").alt,
			CELL.querySelector("img.field").alt,
		] = [
	...
	const falling_orb = () => {
		...
		const FALL_TIMER = setInterval(() => {
			...
			DATA.board.obj[0] = DATA.board.obj[0].map(x =>
	...
	const chain_connect = cell => {
		...
		if (Math.abs(chain_yx.at(-1).y - CELL_Y) > 1) return;
		if (Math.abs(chain_yx.at(-1).x - CELL_X) > 1) return; /*位置チェック*/
